...
SELECT * FROM STOREGE ORDER BY DATE DESC;


INSERT INTO STOREGE(DATE,TITLE,CONTENT,ID)
VALUE (now(),'안녕하세요','1','1');

INSERT INTO COMENT(ID,COMENTS,STOREGE_ID,DATE)
VALUES ('1','1',8,now());
...
SELECT @rownum:=@rownum+1 as rownum, S.*
FROM (SELECT TITLE,DATE,CLICK,ID FROM STOREGE WHERE (@rownum:=0)=0 AND TITLE LIKE '%안녕하세요%' ORDER BY DATE DESC) S


SELECT COUNT(IFNULL(ID,0)) FROM MEMBER_INFO WHERE ID='4444444'
...
## 커뮤니티 페이지 제한
SELECT @rownum:=@rownum+1 as rownum, S.*
FROM (SELECT * FROM STOREGE WHERE (@rownum :=10)=10 ORDER BY DATE DESC) S 
LIMIT 10 OFFSET 10;
